    ...
    fetch('http://localhost:5000/api/posts')
        ...
        .then(data => {
            ...
            data.forEach(post => {
                ...
                if (post.media_url) {
                    if (post.media_url.startsWith('data:image')) {
                        postItem.innerHTML += `<img src="${post.media_url}" alt="Uploaded Media" style="max-width: 100%; height: auto;">`;
                    } else if (post.media_url.startsWith('data:video')) {
                        postItem.innerHTML += `<video controls style="max-width: 100%; height: auto;"><source src="${post.media_url}">Your browser does not support the video tag.</video>`;
                    }
                }
